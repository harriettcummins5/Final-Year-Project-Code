...
# Setup GTAnalysis
gta = GTAnalysis('SMC_data2_binsz/config.yaml', logging={'verbosity': 3})
gta.setup()
...
sed = gta.sed(source, outfile='/home/oslaf/hcummins/high_energy_analysis/HEA_fixed/SMC_data2_binsz/2.1-roi_after_fit_sed.fits',
              bin_index=2, loge_bins=[3.48, 3.8, 4.1, 4.4, 4.7, 5.0, 5.3, 5.6, 6.0],
              make_plots=True)
print(sed)

# View SED table
sed_table = Table.read('/home/oslaf/hcummins/high_energy_analysis/HEA_fixed/SMC_data2_binsz/2.1-roi_after_fit_sed.fits')
...
gta.residmap('2-roi_after_fit_source_removed_below100GeV', model=model, make_plots=True, loge_bounds=[3.48, 5])
gta.tsmap('2-roi_after_fit_source_removed_below100GeV', model=model, make_plots=True, loge_bounds=[3.48, 5], multithread=True)
